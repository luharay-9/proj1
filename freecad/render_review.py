...
P='/Users/harrylu/Downloads/proj1/freecad/'
D=A.openDocument(P+'Shinguard_Redesigned.FCStd')
names=['GuardBase','GuardCover','Adafruit_BNO085_STEMMA_QT_v2','_400mAh_Battery_v2','PCB_Component','Adafruit_ESP32_Feather_V2_v2','Part__Feature355']
...
views=[('ASSEMBLED',True,False,False),('ELECTRONICS IN ORIGINAL POSITIONS',False,False,False),('FITTED SUPPORTS',False,False,True),('COVER LIFTED FOR INSPECTION',True,True,False)]
...
draw.text((70,45),'Shinguard | Rounded trapezoid sandwich',font=ImageFont.truetype(bold,53),fill='#183442')
draw.text((72,116),'CAD geometry | Long-edge switch | 2.4 mm shells | Electronics clear both surfaces',font=ImageFont.truetype(font,25),fill='#536975')
for idx,(title,cover,explode,bare) in enumerate(views):
 ts=[];cs=[]
 for n,(tri,col) in meshes.items():
  if n=='GuardCover' and not cover:continue
  if bare and n!='GuardBase':continue
  tri=tri.copy()
  if n=='GuardCover' and explode:tri[:,:,2]+=42
 ...
 canvas.paste(panel,(x,y));draw.text((x+15,y-38),title,font=ImageFont.truetype(bold,25),fill='#263a47')
 print('Rendered',title,flush=True)
draw.text((75,1645),'Inspection view lifts the cover only; the saved CAD has the cover in its assembled position.',font=ImageFont.truetype(font,23),fill='#536975')
canvas.save(P+'Shinguard_Review.png')
